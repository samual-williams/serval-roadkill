---
title: "Serval roadkill occupancy modelling"
output: html_notebook
---

Here is some preliminary single-species single-season models occupancy modelling I have been doing, to become familiar with how occupancy modelling works. 

```{r}
# Prepare data for occupancy modelling

# rm(list=ls()) # clear memory if necessary
...
hist <- data.frame(hist[,-1], row.names=hist[,1]) # makes road sections into row names 
...
cov.num<-cov[,sapply(cov,is.numeric)] # separate out the numeric covariates for colinearity check, and subsequent standatdisation
library(usdm)
